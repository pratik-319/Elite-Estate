        ...
        <main className='p-3 max-w-4xl mx-auto '>
            ...
            <form onSubmit={handleSubmit} action="" className='flex flex-col sm:flex-row gap-4'>
                ...
                <div className='flex flex-col gap-4 flex-1'>
                    ...
                    <div className="flex flex-col gap-4 flex-1">
                        <input type='text' className='border p-3 rounded' placeholder='Name' id='name' onChange={handleChange} value={formData.name} maxLength='62' minLength='10' required />
                        <textarea type='text' className='border p-3 rounded' placeholder='Description' id='description' onChange={handleChange} value={formData.description} required />
                        <input type='text' className='border p-3 rounded' placeholder='Address' id='address' required onChange={handleChange} value={formData.address} />
                    </div>
                    ...
                    <div className='flex flex-wrap gap-6'>
                        <div className='flex items-center gap-2'>
                            <input type='number' id='bedrooms' className='p-3 border border-grey-300 rounded-lg' onChange={handleChange} value={formData.bedrooms} min='1' max='10' required />
                            <p>Beds</p>
                        </div>
                        <div className='flex items-center gap-2'>
                            <input type='number' id='bathrooms' className='p-3 border border-grey-300 rounded-lg' onChange={handleChange} value={formData.bathrooms} min='1' max='10' required />
                            <p>Baths</p>
                        </div>
                        <div className='flex items-center gap-2'>
                            <input type='number' id='regularPrice' className='p-3 border border-grey-300 rounded-lg' min='50' onChange={handleChange} value={formData.regularPrice} max='1000000' required />
                        ...
                        </div>
                        {
                            formData.offer && (
                                <div className='flex items-center gap-2'>
                                    <input type='number' id='discountPrice' className='p-3 border border-grey-300 rounded-lg' onChange={handleChange} value={formData.discountPrice} min='0' max='1000000' required />
                ...
                <div className='flex flex-col flex-1 gap-4'>
                    ...
                    <div className='flex  gap-4 '>
                        ...
                        <button type='button' disable={uploading} onClick={handelImageSubmit} className='p-3 text-green-700 border border-green-700 rounded uppercase hover:shadow-lg disabled:opacity-80'>{uploading ? 'Uploading.. ' : 'upload'}</button>
                    </div>
                    <p className='text-red-700 text-sm'>{imageUploadError && imageUploadError}</p>
                    {
                        formData.imageUrls.length > 0 && formData.imageUrls.map((url, index) => (
                            <div key={url} className="flex justify-between p-3 border item-center ">
                                <img src={url} alt='listing Image' className='w-32 h-32 object-contain rounded-lg' />
